...
def write_in_batches(data, filename, batch_size=100):
    ...
    with tf.io.TFRecordWriter(record_file) as writer:
        ...
        for i, batch in enumerate(data.take(-1)):
            batch = np.array(batch)
            batch = crop_images(batch, i)
            tf_example = example_test(batch.tobytes())
...
write_in_batches(X_train, 'train_image_cropped.tfrecord', batch_size=128)
write_in_batches(X_test, 'test_image_cropped.tfrecord', batch_size=128)
